...
<div class="ecmGrupoPermissao col-md-12">
    ...
    <fieldset>
        <legend><?= __('Editar Grupo Permissão') ?></legend>
        <?php
            ...
            echo $this->Form->input('mdl_user._ids', ['label'=>__('Usuários Selecionados'), 'options' => $optionsUser]);
            echo $this->Html->tag('/fieldset');

            echo $this->Html->tag('div', null, ['class' => 'input text required']);
            echo $this->Html->tag('label', __('Empresa'), ['for' => 'ecm_alternative_host[_ids]']);
            echo $this->Form->select('ecm_alternative_host_id', $ecmAlternativeHost, ['value' => $ecmGrupoPermissao->ecm_alternative_host_id]);
            echo $this->Html->tag('/div');
