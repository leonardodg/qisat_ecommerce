<div class="ecmCursoPresencialLocal col-md-12">
    ...
    <fieldset>
        <legend><?= __('Edit Ecm Curso Presencial Local') ?></legend>
        <?php
            echo $this->Form->input('nome');
            echo $this->Form->input('mdl_estado_id', ['options' => $mdlEstado, 'label' => 'Estado', 'required' => 'required']);
            echo $this->Form->input('mdl_cidade_id', ['options' => $mdlCidade, 'label' => 'Cidade', 'required' => 'required']);
...
<script>
    $('#mdl-estado-id').change(function() {
        var estado = $(this).val();
        if(estado > 0) {
            $.getJSON('?estado='+estado, function(data){
                $('#mdl-cidade-id').find('option').remove();//.end();
                lista = [];
