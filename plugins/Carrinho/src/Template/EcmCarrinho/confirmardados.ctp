...
<script>
    ...
    $("select[name='formaPagamento']").change(function() {
        ...
        if(($("select[name='formaPagamento'] option:selected").attr('controller') == 'SuperPayRecorrencia')
         || $("select[name='formaPagamento'] option:selected").attr('dataname') == 'api' ){
        ...
        if(formapagamento.indexOf("Cartão") > -1 && $("select[name='formaPagamento'] option:selected").attr('dataname') == 'checkout'){
            $("select[name='valorParcelas']").parent().hide();
            $("#div_operadora").hide();
        } else {
            $("select[name='valorParcelas']").parent().show();
            $("#div_operadora").show();
            $.post("confirmardados", {"formaPagamento": id}, function( data ) {
                ...
                if(Object.keys(data.operadora).length != 0){
                    var operadora = '<div class="input select" id="div_operadora"><label>Selecione a Operadora</label>';
                    var checked = "";
