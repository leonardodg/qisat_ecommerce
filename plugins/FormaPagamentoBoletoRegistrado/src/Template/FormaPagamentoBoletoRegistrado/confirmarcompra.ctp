<div class="ecmCarrinho col-md-12">
    ...
    Para imprimir o boleto bancário clique no botão abaixo:<br>
</fieldset>
    <?= $this->Form->create() ?>
        <?= $this->Form->button(__("Imprimir"), ['name' => 'imprimir', "type" => "button", "onclick" => "location.href='boleto'"]) ?>
        <?= $this->Form->button(__("Avançar >>>"), ['name' => 'avancar', "type" => "button", "onclick" => "location.href='../usuario/listar-usuario'"]) ?>
    <?= $this->Form->end() ?>
